 ...
 * @param {function(Error, string, boolean)} cb - callback(Error, sContainer, containerIsNew)
 */
 ...
 * @param {function(?Error, string, response, containerIsNew)} cb - Callback, returns error or the cloud path, plus the response object, and if a container was created for this add.
 */
Rackit.prototype.add = function (localPath, options, cb) {
	...
	async.parallel({
		...
		function (err, results) {
			...
			var containerIsNew = results.container[1];
			...
			o1._cloudRequest(reqOptions, function (err, res, body) {
				...
				if (!err) {
					o1.hContainers[container].count++;
				}

				cb(err, cloudPath, res, containerIsNew);
			}, function (request) {
